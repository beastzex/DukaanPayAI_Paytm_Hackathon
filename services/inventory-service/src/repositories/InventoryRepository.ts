...
export class InventoryRepository {
  public async createSKU(params: CreateSKUParams): Promise<SKU> {
    const query = `
      INSERT INTO skus (
    ...
    const result = await DatabaseService.query<SKU>(query, values);
  ...
  public async initializeInventory(
    ...
    const result = await DatabaseService.query<Inventory>(query, [
      storeId,
      skuId,
      initialStock,
      reorderPoint,
  ...
  public async updateStockUnits(
    ...
    const query = `
      ...
      RETURNING id
    `;
    await DatabaseService.query(query, [storeId, skuId, deltaUnits]);
    return this.getInventoryItem(storeId, skuId);
  }

  public async getInventoryItem(storeId: string, skuId: string): Promise<InventoryItemDetail | null> {
    const query = `
      SELECT 
    ...
    `;
    const result = await DatabaseService.query<InventoryItemDetail>(query, [storeId, skuId]);
    return result.rows[0] || null;
  }

  public async getStoreInventory(storeId: string): Promise<InventoryItemDetail[]> {
    const query = `
      SELECT 
        i.id as "inventoryId", i.sku_id as "skuId", s.sku_code as "skuCode",
        s.product_name as "productName", s.category,
    ...
    `;
    const result = await DatabaseService.query<InventoryItemDetail>(query, [storeId]);
    return result.rows;
  }

  public async getLowStockItems(storeId: string): Promise<InventoryItemDetail[]> {
    const query = `
      SELECT 
        i.id as "inventoryId", i.sku_id as "skuId", s.sku_code as "skuCode",
        s.product_name as "productName", s.category,
